...
class CNNFeatureExtractor(nn.Module):
    def __init__(self, input_channels=270, output_channels=16, embedding_time_dim=10): # Default embedding_time_dim set to 10
        ...
        # Input: (B, output_channels, 50)
        current_T = 50
        target_T = self.embedding_time_dim
        
        # Ensure stride and kernel are positive
        if target_T <= 0:
            raise ValueError(f"embedding_time_dim must be positive, got {target_T}")
        
        if current_T < target_T:
            final_stride = 1
            final_kernel_size = 1 
            # print(f"Warning: current_T ({current_T}) < target_T ({target_T}). Final conv may not reduce further as intended.")
        elif current_T == target_T:
            final_stride = 1
            final_kernel_size = 1
        else # current_T > target_T
            final_stride = current_T // target_T
            if final_stride == 0: final_stride = 1 # Avoid stride 0
            
            # Calculate kernel size to achieve target_T
            # T_out = floor((T_in - K)/S) + 1  (assuming P=0)
            # target_T = floor((current_T - K)/final_stride) + 1
            # target_T - 1 = floor((current_T - K)/final_stride)
            # For this to work, (current_T - K)/final_stride must be >= target_T - 1
            # and < target_T
            # A common way to set kernel for exact output size with stride S and no padding:
            # K = T_in - (T_out - 1) * S
            final_kernel_size = current_T - (target_T - 1) * final_stride

            if final_kernel_size <= 0: 
                # Fallback if calculated kernel is non-positive
                # This might happen if target_T is very small and final_stride is large
                final_kernel_size = final_stride 
                # Recalculate T_out with this K and S:
                # T_out_recalc = math.floor((current_T - final_kernel_size) / final_stride) + 1
                # If T_out_recalc is not target_T, this strategy might need nn.AdaptiveAvgPool1d(target_T)
                # or a more complex kernel/stride/padding calculation.
                # For now, this fallback is kept simple.
                if final_kernel_size <=0: final_kernel_size = 1 # Ensure kernel is at least 1


        self.final_reduction_conv = nn.Conv1d(output_channels, output_channels, 
                                              kernel_size=final_kernel_size, stride=final_stride, padding=0)
    ...
    def forward(self, x):
        ...
        x = self.hierarchical_dilated_3(x) # Channels: c_initial -> c_hierarchical_out
