...
class DETR_MultiUser_vq(nn.Module):
    ...
    def forward(self, x):
        ...
        quantized_emb_st, quantized_emb, indices = self.vq_layer(continuous_emb)

        memory = self.encoder(quantized_emb_st)

        outputs_class = self.decoder(memory)

        return outputs_class, continuous_emb, quantized_emb, indices

    def compute_vq_loss(self, continuous_emb, quantized_emb):
        return self.vq_layer.compute_loss(continuous_emb, quantized_emb)


class DETR_MultiUser_RVQ(nn.Module):
    """
    DETR model with Residual Vector Quantization (RVQ).
    ...
    def __init__(self, var_x_shape, features_dim=20, embedding_time_dim=100, num_decoder_layers=12,
        ...
        self.rvq_layer = ResidualVectorQuantizer(
            ...
            embedding_dim=preset["nn"]["d_embedding"],
            commitment_cost=commitment_cost,
            initial_embeddings_first_layer=codebook_initial_embeddings
        )
...
def run_that_detrVQ(data_train_x,
                     data_train_y,
                     data_test_x,
                     data_test_y,
                     var_repeat=10):
    ...
    #
    var_macs, var_params = get_model_complexity_info(DETR_MultiUser_vq(var_x_shape,
    ...
    for var_r in range(var_repeat):
        ...
        #
        model_detrVQ = DETR_MultiUser_vq(var_x_shape,
                                    n_attention_heads=preset["nn"]["n_attention_heads"],
                                    features_dim=preset["nn"]["d_embedding"],
                                    embedding_time_dim=preset["nn"]["token_length"],
                                    num_decoder_layers=preset["nn"]["num_decoder_layers"],
        ...
        ## ---------------------------------------- Train -----------------------------------------
        #
        var_best_weight = train_vq_func(model=model_detrVQ,
                                optimizer=optimizer,
                                loss=loss,
        ...
        if preset.get("save_model"):
            save_model_components(preset, model_detrVQ)
        #
        ...
        ## ---------------------------------------- Test ------------------------------------------
        #
        model_detrVQ.load_state_dict(var_best_weight)
        #
        with torch.no_grad():
            outputs_class, _, _, _ = model_detrVQ(torch.from_numpy(data_test_x).to(device))
        #
    ...
    log_random_attention_weights_final(model_detrVQ, np.argmax(predict_test_y[-1], axis=-1), np.argmax(data_test_y, axis=-1), 1000000000)
...
def run_that_detrRVQ(data_train_x,
                     data_train_y,
                     data_test_x,
                     data_test_y,
                     var_repeat=10):
    ...
    var_macs, var_params = get_model_complexity_info(DETR_MultiUser_RVQ(var_x_shape,
                                    n_attention_heads=preset["nn"]["n_attention_heads"],
                                    features_dim=preset["nn"]["d_embedding"],
                                    embedding_time_dim=preset["nn"]["token_length"],
                                    num_decoder_layers=preset["nn"]["num_decoder_layers"],
    ...
    for var_r in range(var_repeat):
        ...
        torch.random.manual_seed(var_r + 39)
        #
        model_detrRVQ = DETR_MultiUser_RVQ(var_x_shape,
                                    n_attention_heads=preset["nn"]["n_attention_heads"],
                                    features_dim=preset["nn"]["d_embedding"],
                                    embedding_time_dim=preset["nn"]["token_length"],
                                    num_decoder_layers=preset["nn"]["num_decoder_layers"],
        ...
        ## ---------------------------------------- Train -----------------------------------------
        #
        var_best_weight = train_rvq_func(model=model_detrRVQ,
                                optimizer=optimizer,
                                loss=loss,
                                data_train_set=data_train_set,
        ...
        # Save model components based on scenario
        if preset.get("save_model"):
            save_model_components(preset, model_detrRVQ)
        #
        var_time_1 = time.time()
        #
        ...
        ## ---------------------------------------- Test ------------------------------------------
        #
        model_detrRVQ.load_state_dict(var_best_weight)
        #
        with torch.no_grad():
            data_test_x_tensor = torch.from_numpy(data_test_x).to(device)
            outputs_class, continuous_emb, quantized_emb, all_indices, all_quantized_layers = model_detrRVQ(data_test_x_tensor)
        #
        predict_test_y = outputs_class.detach().cpu().numpy()
        #
        var_time_2 = time.time()
    ...
    for layer_idx_num, layer_idx in enumerate(layers_idxs):
        wandb.log({
            ...
            f"test_results/{layer_idx}/avg_precision": sum(result_precision[layer_idx_num]) / len(
                result_precision[layer_idx_num]),
            f"test_results/{layer_idx}/avg_recall": sum(result_recall[layer_idx_num]) / len(
                result_recall[layer_idx_num]),
            f"test_results/{layer_idx}/avg_f1_score": sum(result_f1_score[layer_idx_num]) / len(
                result_f1_score[layer_idx_num]),
            f"test_results/{layer_idx}/avg_count_error": sum(result_avg_count_error[layer_idx_num]) / len(
                result_avg_count_error[layer_idx_num]),
            f"test_results/{layer_idx}/avg_accuracy": sum(result_accuracy[layer_idx_num]) / len(
                result_accuracy[layer_idx_num])
    ...
    log_random_attention_weights_final(model_detrRVQ, np.argmax(predict_test_y[-1], axis=-1), np.argmax(data_test_y, axis=-1), 1000000000)
