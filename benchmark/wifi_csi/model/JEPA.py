...
class Predictor(nn.Module):
    def __init__(self, config):
        ...
        self.encoder_d_model = config["d_embedding"]
...
class JEPA_Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.online_transformer_encoder = Transformer_Encoder(
            d_model=config["d_embedding"],             # Feature dimension of tokens.
    ...
    def _update_ema(self, iter):
        ...
        ema_decay = 1 - (1 - self.ema_decay_base) * (np.cos(np.pi * iter / self.config["epoch"]) + 1) / 2
    ...
    def _extract_segments_from_x(self, x_item_full_view, segment_indices_list_for_item):
        ...
        if not segment_indices_list_for_item:
             return torch.empty(0, self.config["segment_length"], 270,
        ...
        for i in range(batch_size):
            ...
            if not item_context_segment_indices: # If no segments, then no tokens
                ...
                all_extracted_cnn_tokens_batch.append(
                    torch.empty(0, self.config["d_embedding"], device=x_item_full_view.device))
                continue
            ...
            all_extracted_cnn_tokens_batch.append(
                cnn_tokens_item.reshape(num_item_context_tokens_from_cnn, self.config["d_embedding"]))

        # Pad the extracted CNN tokens to max_context_tokens_in_batch
        # This tensor holds the actual token embeddings.
        padded_context_tokens = torch.zeros(batch_size, max_context_tokens_in_batch, self.config["d_embedding"],
    ...
    def _process_full_view_for_target_encoder(self, x_full_view_batch, cnn_extractor, transformer_encoder):
        ...
        cnn_out_channels = self.config["d_embedding"]
    ...
    Note: This method is optimized to avoid nested list operations and directly work
        ...
        cnn_output_channels = self.config["d_embedding"]
    ...
    def forward(self, x_raw_input):
        ...
        # z_context_online shape: (batch_size, max_context_tokens_in_batch, self.config["d_embedding"])
...
def train_jepa(jepa_model, dataloader, optimizer, device, config, num_epochs=10, 
    ...
    run_name = f"JEPA_SSL_training_{'+'.join(config['environment'])}"
    run = wandb.init(
        project="jepa_ssl",
        name=run_name,
        config=config,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--epochs', type=int, default=JEPA_CONFIG["epoch"], help='Number of epochs to train')
    parser.add_argument('--batch-size', type=int, default=16, help='Batch size for training')
    parser.add_argument('--envs', nargs='+', default=JEPA_CONFIG["environment"],
