...
#
##
class Crowd_counting_THAT(torch.nn.Module):
    #
    ##
    def __init__(self,
                 var_x_shape,
        ...
        #
        ##
        super(Crowd_counting_THAT, self).__init__()
        #
        ...
        #
        ##
        self.layer_out1 = torch.nn.Linear(256 + 32, 128)
        self.last_dropout = torch.nn.Dropout(0.1)

        self.layer_out2 = torch.nn.Linear(128, 6)
    ...
    def forward(self,
        ...
        #
        var_t = self.layer_out1(var_t)
        var_t = self.layer_leakyrelu(var_t)
        var_t = self.last_dropout(var_t)
        var_out = self.layer_out2(var_t)
        #
        ##
        return var_out
...
#
##
def run_crowd_counting_THAT(data_train_x,
             data_train_y,
             data_test_x,
             data_test_y,
             var_repeat=10):
    ...
    #
    ##
    var_macs, var_params = get_model_complexity_info(Crowd_counting_THAT(var_x_shape, var_y_shape),
    ...
    for var_r in range(var_repeat):
        ...
        ##
        print("Repeat", var_r)
        name_run = f"crowd_counting_{var_r}_" + "_".join(preset["data"]["environment"])
        ...
        #
        model_that = Crowd_counting_THAT(var_x_shape, var_y_shape).to(device)
        #
        ...
        result_MCE.append(MCE)
        print(f"Current Results for MCE: {MCE}")
    ...
    result = {
        'avg_MCE': sum(result_MCE) / len(result_MCE),
